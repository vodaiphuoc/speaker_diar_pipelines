...
class Speaker_Diarization_Offline(object):
    ...
    def __init__(
        self, model_id: str = "nvidia/diar_sortformer_4spk-v1", device: str = "cuda"
    ):
        self._diar_model = SortformerEncLabelModel.from_pretrained(
            model_id, map_location=torch.device(device)
        )
    ...
    def forward(
        ...
        for ele in predicted_segments:
            ...
            assert start_timestamp < end_timestamp, (
                f"Found start_timestamp: {start_timestamp}, end_timestamp: {end_timestamp}"
            )
            results.append(
                {
        ...
        assert len(results) != 0, "Found empty results"
...
class Speaker_Diarization_Online(object):
    ...
    def custom_export(self, onnx_checkpoint: str = "/mydata/test.onnx"):
        ...
        def custom_concat_and_pad(
            embs: List[torch.Tensor], lengths: List[torch.Tensor]
        ):
            """
            ...
            output = torch.zeros(
                batch_size, sig_length, emb_dim, device=device, dtype=dtype
            )
            ...
            for emb, length in zip(embs, lengths):
                ...
                for batch_idx in range(batch_size):
                    ...
                    output[batch_idx, s:e] = emb[batch_idx, :l]

                start_indices = end_indices

            return output, total_lengths

        self._diar_model.streaming_input_examples = types.MethodType(
            custom_streaming_input_examples, self._diar_model
        )
