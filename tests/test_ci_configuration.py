...
class DockerAndWorkflowTest(unittest.TestCase):
    ...
    def test_pipeline_calibration_supports_qwen3_asr_backend(self):
        ...
        self.assertIn("def run_pipeline_calibration_remote(", modal_runner)
        self.assertIn("asr_backend: str", modal_runner)
        self.assertIn("calibration_test_target: str", modal_runner)
        self.assertIn("run_pipeline_calibration_remote.remote(", modal_runner)
        self.assertIn("PIPELINE_ASR_BACKEND,", modal_runner)
        self.assertIn("PIPELINE_CALIBRATION_TEST_TARGET,", modal_runner)
